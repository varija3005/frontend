...
const SpecialSection = () => {
    ...
    useEffect(() => {
        axios.get('http://localhost:5000/api/prod/1')
        .then(res => {
            setProd(res.data)
        })
        .catch(err => {
    ...
    return (
        <div className='special-section'>
            ...
            <div className='product-details'>
                ...
                <p className='text'>Grab it now at a special discounted price for a limited time!</p>
                <div className='price-container'>
                    <div className='prices'>
                        <p className='strikeThrough'>₹. {prod.mrp}</p>
                        <p className='price'>₹. {prod[location.pathname.slice(1)]}</p>
                    </div>
                    <Link to={`/product${location.pathname}/1`}><button className='button'>Check out!</button></Link>
                </div>
            </div>
        </div>
    )
